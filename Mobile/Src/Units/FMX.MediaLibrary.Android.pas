...
type
  ...
  TImageManagerAndroid = class sealed(TInterfacedObject, IFMXCameraService,
    IFMXTakenImageService, IFMXPhotoLibrary)
    private
      FParams: TParamsPhotoQuery;
      { FMX Messages System }
      procedure DidReceiveBitmap(const Sender: TObject; const M: TMessage);
      procedure DidCancelReceiveBitmap(const Sender: TObject;
        const M: TMessage);
    ...
    public
      ...
      { IFMXCameraService }
      procedure TakePhoto(const AControl: TControl;
        const ARequiredResolution: TSize; const AEditable: Boolean;
        const AOnDidFinishTaking: TOnDidFinishTaking;
        const AOnDidCancelTaking: TOnDidCancelTaking); overload;
      procedure TakePhoto(const AControl: TControl;
        const AParams: TParamsPhotoQuery); overload;
      { IFMXPhotoLibrary }
      ...
      procedure TakeImageFromLibrary(const AControl: TControl;
        const ARequiredResolution: TSize; const AEditable: Boolean;
        const AOnDidFinishTaking: TOnDidFinishTaking;
        const AOnDidCancelTaking: TOnDidCancelTaking); overload;
      procedure TakeImageFromLibrary(const AControl: TControl;
  ...
  TSharingManagerAndroid = class(TInterfacedObject,
    ...
    public
      { IFMXShareSheetActionsService }
      procedure Share(const AControl: TControl; const AText: string;
...
begin
  ...
  TPlatformServices.Current.AddPlatformService(IFMXTakenImageService,
    ImageManager);
  if TAndroidHelper.HasSystemService(TJPackageManager.JavaClass.FEATURE_CAMERA)
    or TAndroidHelper.HasSystemService
    (TJPackageManager.JavaClass.FEATURE_CAMERA_FRONT) then
    TPlatformServices.Current.AddPlatformService(IFMXCameraService,
      ImageManager);

  SharingManager := TSharingManagerAndroid.Create;
  TPlatformServices.Current.AddPlatformService(IFMXShareSheetActionsService,
    SharingManager);
end;
...
begin
  TMessageManager.DefaultManager.SubscribeToMessage
    (TMessageCancelReceivingImage, DidCancelReceiveBitmap);
...
begin
  TMessageManager.DefaultManager.Unsubscribe(TMessageReceivedImagePath,
    DidReceiveBitmap);
  TMessageManager.DefaultManager.Unsubscribe(TMessageCancelReceivingImage,
    DidCancelReceiveBitmap);
  inherited;
end;

procedure TImageManagerAndroid.DidCancelReceiveBitmap(const Sender: TObject;
  const M: TMessage);
begin
  if Assigned(FParams.OnDidCancelTaking) then
    ...
    TMessageManager.DefaultManager.SendMessage(Self,
      TMessageDidCancelTaking.Create);
end;

procedure TImageManagerAndroid.DidReceiveBitmap(const Sender: TObject;
  const M: TMessage);
var
  ImagePath: string;
  Photo: TBitmap;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        if RequestCode = TJFMXMediaLibrary.JavaClass.ACTION_TAKE_IMAGE_FROM_CAMERA
        then
          TMessageManager.DefaultManager.SendMessage(Self,
            TMessageDidFinishTakingImageFromCamera.Create(Photo));
        if RequestCode = TJFMXMediaLibrary.JavaClass.ACTION_TAKE_IMAGE_FROM_LIBRARY
        then
          TMessageManager.DefaultManager.SendMessage(Self,
            TMessageDidFinishTakingImageFromLibrary.Create(Photo));
      end;
    finally
      Photo.Free;
    end;
...
procedure TImageManagerAndroid.AddImageToSavedPhotosAlbum
  (const ABitmap: TBitmap; const AWriteImageCompletionEvent
  : TWriteImageCompletionEvent = nil);
  ...
  var
    AlbumPath: JFile;
  begin
    AlbumPath := TJUtils.JavaClass.getAlbumDir
      (StringToJString(SSavedPhotoAlbum));
...
begin
  try
    ...
    Intent := TJIntent.JavaClass.init
      (TJIntent.JavaClass.ACTION_MEDIA_SCANNER_SCAN_FILE);
...
function TImageManagerAndroid.ApproximateAdmissibleImageSize
  (const ASize: TSize): TSize;
begin
  Result.cx := Min(TCanvasManager.DefaultCanvas.GetAttribute
    (TCanvasAttribute.MaxBitmapSize), ASize.Width);
  Result.cy := Min(TCanvasManager.DefaultCanvas.GetAttribute
    (TCanvasAttribute.MaxBitmapSize), ASize.Height);
end;
...
begin
  ...
  FParams.RequiredResolution := ApproximateAdmissibleImageSize
    (ARequiredResolution);
  FParams.NeedSaveToAlbum := False;
  FParams.OnDidFinishTaking := AOnDidFinishTaking;
  FParams.OnDidCancelTaking := AOnDidCancelTaking;
  MainActivity.getMediaLibrary.TakeImageFromLibrary
    (FParams.RequiredResolution.Width, FParams.RequiredResolution.Height,
    FParams.Editable);
end;
...
begin
  ...
  FParams := AParams;
  FParams.RequiredResolution := ApproximateAdmissibleImageSize
    (FParams.RequiredResolution);
  MainActivity.getMediaLibrary.TakeImageFromLibrary
    (FParams.RequiredResolution.Width, FParams.RequiredResolution.Height,
    FParams.Editable);
end;

procedure TImageManagerAndroid.TakePhoto(const AControl: TControl;
  const AParams: TParamsPhotoQuery);
begin
  if TJBuild_VERSION.JavaClass.SDK_INT > 32 then
  begin
  ...
  FParams.RequiredResolution := ApproximateAdmissibleImageSize
    (FParams.RequiredResolution);
  MainActivity.getMediaLibrary.takeImageFromCamera
    (FParams.RequiredResolution.Width, FParams.RequiredResolution.Height,
...
procedure TSharingManagerAndroid.AddImage(var AIntent: JIntent;
  const AImage: TBitmap);
var
  ImageFile: JFile;
  ImageUri: Jnet_Uri;
...
begin
  ...
  ImageFile := TJFile.JavaClass.createTempFile(StringToJString('attachment'),
    StringToJString('.png'), OutputDir);
  ...
  if FDataType = TSharingDataType.Image then
    AIntent.putExtra(TJIntent.JavaClass.EXTRA_STREAM,
      TJParcelable.Wrap((ImageUri as ILocalObject).GetObjectID))
  else
  begin
    ...
    AIntent.putParcelableArrayListExtra(TJIntent.JavaClass.EXTRA_STREAM,
      ArrayList);
  end;
end;

procedure TSharingManagerAndroid.AddText(var AIntent: JIntent;
  const AText: string);
begin
  Assert(AIntent <> nil);
...
procedure TSharingManagerAndroid.Share(const AControl: TControl;
  const AText: string; const AImage: TBitmap);
